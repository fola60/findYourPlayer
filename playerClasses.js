    ...
    (err,res)=>{
        if(!err){
            ...
            for(let i = 0; i < res.rows.length;i++){
                let df_bpa = (parseFloat(res.rows[i].cmp90per) + parseFloat(res.rows[i].sho_cmppctper) + parseFloat(res.rows[i].med_cmppctper) + parseFloat(res.rows[i].lon_cmppctper) + parseFloat(res.rows[i].p_rec90per) + parseFloat(res.rows[i].prg_prec90per) + parseFloat(res.rows[i].kp90per)) / 6;
